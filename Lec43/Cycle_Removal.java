...
//O(1)
	...
	public void removeCycle1() {
		Node meetup = hasCycle(); // meetup me wo node aega jo upr wala hasCycle method return krega , see in Lec42 Linked_List_Cycle class
...
//******************************************************                        ***************************************                        **********************
	...
	public void remveCycle2() {
		Node meetup = hasCycle(); // yha upr jo hasCycle wala methode se meetup me wo node aegega jha fast and slow dono note ek node pr mil rhe ahi eg 7|8k wala node
		if(meetup == null) { // agar LinkedList me cycle nhi hai then
			return; // return
		}
		
		//step 1 see in notebook // to count the nodes in that are in cycle
		int count = 1; // count krne ke liye
		Node temp = meetup; // meetup wale node se count start krne ke liye
		while (temp.next != meetup) { //yha temp meetup point se start hoga and cycle ghumega //jb tk temp wale node ke next me meetup wale node ka address nhi ata then
			count++; // count +1
		...
		for (int i = 0; i < count; i++) { // Pin pointer ko jitna count hai 
			pin = pin.next; //utna age bdha diya jisse wo meetup point pr a gya			
		}		
		...
		pin.next = null; //Step 4 // upr while loop ke khtm hote hi ye line chlegi and pointer ke next me null krdenge now Linked list se cycle remove hojaegi	
	}
...
//******************************************************                        ***************************************                        **********************
	...
	public void fylodcycleremoval() {
		...
		Node pin = meetup; // pin pointer me meetup wala node aega and meetup me wo node hai jha fast and slow dono note ek node pr mil rhe ahi eg 7|8k wala node
		                   // step 1 and 2 ke bdle hmne yha - ek "pin" name pointer le liye jo ki "meetup" point se start hoga esa krne se count lene and age bdhane ka step skip kr diya 
		//further steps
		Node start = head; //  start - head ko point krega 
		while(start.next != pin.next) { //Step 4 // jb tk start and fast dono ke node ke next me same node ka address nhi ata tbtk
			start = start.next; //Step 3 // start ko ek se age bdha
...
//******************************************************                        ***************************************                        **********************
	 ...
	 public void Display() {       //ye code Lec 41 and LinkedList class se liya h
			Node temp = this.head; // 1st iteration me temp me 14k address aega then see syso wali line and notebook  
			                       //2nd iteration me "temp = temp.next;" hokr temp 2k location ko point krega  // head ko temp me rkh rhe h //agar temp nhi lete then head last last node print hote hote head null ko point krta and pura LinkedList khali ho jata
			while (temp != null) { // it means temp jb tk null nhi hota(last node pr nhi phuchta) tb tk
				System.out.print(temp.data + "-->"); // temp me jo 14k address ka jo data h wo print hoga eg:- 30 then see bolow  
				                                     // 2nd iteration me 2k location ke coresponding jo data hoga wo print hoga eg:-20 // temp.data me head wala element hoga (eg:- 10) then niche // agar temp.data <- iske bdle temp.next likhte then address print krta
		...
		public static void main(String[] args) throws Exception {            //isi class me "main class" bna di , upr wale methods ko run krne ke liye client class bnane ki jrurat nhi pdi
		  ...
		  //cl.remveCycle2();  // LinkedList se cycle remove hogi isse
			cl.fylodcycleremoval(); // LinkedList se cycle remove hogi isse
			cl.Display();      // ab loop infinite nhi chlega because ab fylodcycleremoval wale method se cycle remove ho gyi hai
    }
}
