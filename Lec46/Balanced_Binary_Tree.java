...
//  upr wala code from leetcode 
	...
	class Solution {
		...
		public BalancedPair TreeBalanced(TreeNode root) {
			 if(root == null) { //niche wala recursion call lgega and end me leaf node ke next null pr phuchte hi //Base Case
				 return new BalancedPair(); // BalancedPair me (isbal me -> true) &&  (ht me -> 1) return hoga
			 }
			BalancedPair lbp = TreeBalanced(root.left);
			BalancedPair rbp = TreeBalanced(root.right);
			
			BalancedPair sbp = new BalancedPair();
			sbp.ht = Math.max(lbp.ht, rbp.ht)+1;  //height claculate krne ke liye
			int bf = Math.abs(lbp.ht - rbp.ht);   // " abs " ek positive value dega 
			   
			sbp.isbal = lbp.isbal   &&   rbp.isbal   &&   bf<=1;   // isbal calculate kr rhe h
