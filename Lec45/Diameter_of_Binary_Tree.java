...
public class Diameter_of_Binary_Tree {  //complexity -> O(N^2)  // Leetcode - 543
 ...
 // upr wala code leetcode se uthaya h // see in notebook
		    	
		    	if(root == null) { //base case
		    		return 0;
		    	}
		    	int ld = diameterOfBinaryTree(root.left); // ye left tree ka diameter ke liye recursive call   // ld is left diameter
		    	int rd = diameterOfBinaryTree(root.right);// ye right tree ka diameter ke liye recursive call  // rd is right diameter
		    	int sd = height(root.left)+ height(root.right)+2; // root se pass hone wala diameter           // sd is self diameter
		    	
		    	return Math.max(ld, Math.max(rd, sd)); // teeno me se jiska lenght sbse maximum hoga wo return hokr o/p bn jaega
		    }		    
			private int height(TreeNode node) { //iss method ko self diameter calculate krne ke liye use ke rhe h// upr "public class TreeNode" h  				
