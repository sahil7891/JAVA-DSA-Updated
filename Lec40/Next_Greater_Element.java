...
public class Next_Greater_Element { //Comlexity == O(N)
	...
	public static void main(String[] args) {
		...
		public static void NGE(int[] arr) {
			...
			int [] nge = new int [arr.length]; // Step 1 // "nge" name ka ek Array leliya
			Stack<Integer> st = new Stack<>(); // "st" name ka ek Stack leliya
			
			for (int i = 0; i < arr.length; i++) { // i= index(0th index) // ye loop jitne elements h utni br chl rha hai isliye comlexcity O(N) hai
// &&  Logical AND: returns true when both conditions are true. (&& means ki dono hi condition true honi chahiye then tbi while loop ke niche ki chlega eg:- int ii and nge[ii] wali line and agar true nhi hui then while loop ke bhr wali line chlegi eg:- st.push(i); wali line)
// ||  Logical OR : returns true if at least one condition is true.				
				while(!st.isEmpty() &&  arr[i]> arr[st.peek()]) { //!st.isEmpty() - ye check krega ki stack khali h ki nhi // isEmpty && i>peek <-- jb ye dono true honge then - while loop ke niche ye dono line --> ii=st.pop and ii= arr[i]  chalegi
					//1st iteration me-> i = 0 me 0th index(11) ajayega eg:- i=11 and (!st.isEmpty())-then ye stack khali hai isliye -(st.push(i))<- ye line chlegi and iski wjh se Stack me index no 0 (11) jaye
