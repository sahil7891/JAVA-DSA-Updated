...
public class Time_and_Space_Complexity {
	...
	public static void main(String[] args) {
		...
		int n = 1000;
		int m = 1000000000;
		System.out.println(3+4); //in this time complexity is => "O(1)"  means order of 1 because ise frk nhi pd rha ki n ki value kya hai
		System.out.println("bye");
		System.out.println("bye hey");
		System.out.println("bye");// constant time "O(1)" 
		...
		while (i<n) {
			System.out.println("Hey");
			i*=2;  // yha multiplication ka case h then then "Comlexity-->log" me hogi eg:-  O(log(N))
			       // Time Complexity => O(log(N))  //order of log N
		}
		//---------------------------------------
		while (n>0) {
			System.out.println("Hey");
			n/=2;     // division ke ka case h isliye complexity --> log me hogi eg:-  O(log(N))
			//O(log(N)
		}
		//------------------------
		while (i<=n) {
		...
		for (i = 1; i <= n; i++) {          // n times chl rha hai
			for (int j = 1; j <= n; j++) {  //ye under wala loop bahar wale variable pr depend nhi kr rha then == ye loop n times chal rha hai and uper wala loop bhi n times then time complexity = n squere
				//dono loop independent hai isliye constant work ho rha hai then complexity = n*n means N square (N^2)
