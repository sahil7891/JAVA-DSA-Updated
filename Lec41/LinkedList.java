...
public class LinkedList {
	...
	public class Node{ // ikathe 2 information ko combine krke rkhna hai(eg:- data & address) isliye class bnana pda // Node bnaya hai to know that "data"(eg:-10) kya hai and "address"(eg- 1k) kya hai see below data ke liye "int data" liye h and address ke liye  Node next likha h                       
		...
		Node next;  //"next" me address yad rkhege eg:- 1k,2k // "next" Node ka address yad rkh rha hai // address ka data type "Node" type ka hai 
		            //for eg:- ek Node me (int data me 30 hoga)&(Node next me 2k(next node ka address hoga))
		public Node(int data) { //Node ka constructor bna diya taki client class me iss class ke methods ko use kr ske 
	...
	public void addfirst(int item) { //item me element denge client class ke through eg:- 10,20,30 // comlexity - O(1) bcouse koi loop nhi lg rha and constant time lg rha hai
	...
	public void addlast(int item) { // iss method se hm element ko last me add krte hai eg:- 30 ,20 ,10 ke bad "40" add krenge //item me 40 denge // comlexity - O(1) bcouse koi loop nhi lg rha and constant time lg rha hai
	...
	public int getatindex(int k) throws Exception { // node ke index pr jo data hai uuse get kr rhe hai and k m index no. denge
		...
		return GetNode(k).data;  //niche wala GetNode method call ho rha hai see below //ye node ka data return krega(GetNode(k).data;) agar yha data ke bdle next likha hota ye address return krta // k me node ka address aega and uss address ka data return hoga
		}
	...
	public int addatanyindex( int item, int k) throws Exception {  //k me index no. btenge ki konse index pr node add krena hai and item me element denge //LinkedList me kisi node ko nikal skte hai  //comlexity - O(N) bcause  GetNode function ko call kiya h and usme loop hai
		 if(k<0 || k> size){ // Exception ke liyeh ye line and agar k ki value range se bhr hogi
	    	  throw new Exception ("Index out of Bound hai pgl k  ki value range me de");
		 }
