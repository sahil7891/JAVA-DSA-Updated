...
public class Stack_Using_Queue {    // using "pop" wala sir(sir wala) krwa rhe hai see in notebook
		...
		public void push(int item) throws Exception { // comlexity -> O(N)  // see steps in notebook for push method
			DynamicQueue temp = new DynamicQueue();   //Step 1 // ek temp name ka DynamicQueue le liya (ye temprorat DynamicQueue h)
				while(! this.data.isEmpty()) {    // ! ki help se jb tk Stack me element hoga tb tk loop chlega
					temp.Enqueue(data.Dequeue()); //Step 2 // temp.Enqueue ki help se temporary wali Stack(DynamicQueue) me element dal rhe hai 
					                              // and data.Dequeue ki help se DynamicQueue se data ko nikal rhe hai and jb tk Stack me element hoga tb tk loop chlega				
				}
